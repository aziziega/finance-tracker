...
export function FormTransaction({ onComplete, editTransaction }: AddTransactionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Always include categoryId, even for transfer (can be null)
      if (transactionType === 'transfer') {
        payload.categoryId = null  // Explicitly set to null for TRANSFER
        payload.toAccountId = selectedToAccount
      } else {
        payload.categoryId = selectedCategory || null
      }
      ...
      if (response.ok) {
        ...
        if (error.error?.includes('not installed') || error.error?.includes('does not exist')) {
          toast.error('Database Setup Required', {
            description: 'Stored procedures are not installed. Please contact administrator or check TROUBLESHOOTING_DATABASE_ERROR.md',
            duration: 6000,
          })
        } else if (error.error?.includes('operator does not exist') || error.error?.includes('uuid')) {
          toast.error('Data Type Error', {
            description: 'Invalid account or category ID. Please refresh the page and try again.',
            duration: 5000,
          })
        } else if (error.error === 'INSUFFICIENT_BALANCE') {
          toast.error('Insufficient Balance', {
            description: error.message || 'Your account does not have enough balance for this transaction',
            duration: 5000,
          })
        } else if (error.error === 'UNAUTHORIZED' || error.error === 'ACCOUNT_NOT_FOUND') {
          toast.error('Account Error', {
            description: error.message || 'Selected account is not available or unauthorized',
            duration: 5000,
          })
        } else if (error.error === 'MISSING_DESTINATION') {
          toast.error('Missing Destination', {
            description: 'Please select a destination wallet for transfer',
            duration: 5000,
          })
        } else if (error.error === 'SAME_ACCOUNT') {
